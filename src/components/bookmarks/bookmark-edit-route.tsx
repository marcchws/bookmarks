...
interface BookmarkEditRouteProps {
  id: string
}

/**
 * REQ-8, REQ-9
 * Fetches the bookmark by ID and passes its values as defaultValues to BookmarkForm.
 * Handles loading + error states before rendering the form.
 */
export function BookmarkEditRoute({ id }: BookmarkEditRouteProps) {
  ...
  if (isLoading) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-8">
        ...
        >
          {/* Title skeleton */}
          <div className="h-8 w-48 animate-pulse bg-surface-container-high motion-reduce:animate-none" />
          {/* Fields skeleton */}
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex flex-col gap-1.5">
              <div className="h-3 w-20 animate-pulse bg-surface-container-high motion-reduce:animate-none" />
              <div className="h-11 w-full animate-pulse bg-surface-container-high motion-reduce:animate-none" />
            </div>
          ))}
          <span className="sr-only">Loading bookmark for editing…</span>
        </div>
      </main>
    )
  }
